  ...
  async (
    ...
    try {
      ...
      if (!userInfo?.email || userInfo?.email === undefined) {
        return {
          error: "Please login in order to add pokemon to your collection.",
        };
      }
      const index = userPokemons.findIndex(
        (userPokemon: userPokemonsType) => userPokemon.name === pokemon.name
      );
      ...
      if (index === -1) {
        ...
        return { success: `${pokemon.name} added to your collection.` };
      } else {
        dispatch(setToast(`${pokemon.name} already part of your collection.`));
        return { success: `${pokemon.name} already part of your collection.` };
      }
    } catch (err) {
      console.log({ err });
